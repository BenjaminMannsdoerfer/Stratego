...
class ControllerSpec extends WordSpec with Matchers {
  "A Controller" when {
    "observed by an Observer" should {
      ...
      "notify its Observer after a character moved down" in {
        controller.moveDown(0,0)
        observer.updated should be(true)
      }
      "notify its Observer after a character moved up" in {
        controller.moveUp(3,0)
        observer.updated should be(true)
      }
      "notify its Observer after a character moved to the right" in {
        controller.moveRight(0,2)
        observer.updated should be(true)
      }
      "notify its Observer after a character moved to the left" in {
        controller.moveLeft(0,1)
        observer.updated should be(true)
      }
